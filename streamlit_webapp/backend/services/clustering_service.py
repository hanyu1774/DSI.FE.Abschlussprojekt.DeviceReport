...
TICKETS_TEXT_QUERY = """
    SELECT t.ticket_id AS id, tt.name AS ticket_type, t.description,
...
def cluster_tickets(connection_string: str, n_clusters: int) -> list[dict]:
    df = load_dataframe(TICKETS_TEXT_QUERY, connection_string)
    n_tickets = len(df)
    if n_tickets == 0:
        return []

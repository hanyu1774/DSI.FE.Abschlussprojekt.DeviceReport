...
deutschem Label - fuer konsistente `st.badge`-Anzeigen im ganzen Frontend.

Nutzt bewusst natives `st.badge` mit den ueber `.streamlit/config.toml`
...
def status_badge(status: str) -> None:
    st.badge(status, icon=STATUS_ICONS.get(status), color=STATUS_COLORS.get(status, "gray"))

...
class ApiClient:
    ...
    def get(self, endpoint: str, params: dict[str, Any] | None = None) -> Any:
        try:
            ...
            st.error("Die API-Anfrage hat zu lange gedauert.", icon=":material/timer_off:")
        except requests.HTTPError:
            st.error(f"API-Fehler: {response.text}", icon=":material/error:")
        except requests.RequestException as error:
            st.error(f"Verbindungsfehler: {error}", icon=":material/error:")
        return None
...
@st.cache_data(ttl="30s", max_entries=50)
def load_tickets(
    limit: int = 100,
